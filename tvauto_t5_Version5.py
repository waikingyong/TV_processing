...
def find_preliminary_files(directory):
    """
    Finds, filters, and sorts preliminary WellCAD files for processing.

    Args:
        directory (str): The path to the directory containing televiewer files.

    Returns:
        list: A sorted list of file paths to be processed. Returns an empty
              list if the directory cannot be parsed.
    """
    try:
        tvfiles = [os.path.join(directory, f) for f in os.listdir(directory)]
        exts = ('.wcl', '.WCL', '.Wcl')
...
def print_summary(directory, rejected_files):
    """
    ...
    Args:
        directory (str): The path to the directory containing televiewer files.
        rejected_files (list): A list of filenames that were rejected during
                                 the interactive azimuth filtering step.
    """
...
def process_borehole_file(tvfile, app):
    """
    ...
    Returns:
        str or None: The path of the saved output file if successful, otherwise None.
    """
    ...
    if should_adjust_visible_depth(log_data_for_check):
        ...
        if not no_rotation:
            ...
            class GraphApp:
                ...
                def _layout_dbscan_params(self):
                    ...
                    self.dbscan_message = ttk.Label(self.paramframe, text=("Avoid using large # samples if it significantly alters background trend.\n" "Recommend : Start small and gradually increase samples until big mag spikes removed.\n" "Then smooth residual high-freq jitter using post-proc alpha trim."), style="App.TLabel", wraplength=1000, foreground="#a73f00")
...
class MainApp:
    """The main application GUI to select and process files."""
    def __init__(self, root):
        self.root = root
        self.root.title("Televiewer Processor")
        ...
        self.select_button = ttk.Button(
            ...
            command=self.select_and_process_files,
            style="App.TButton"
        )
        self.select_button.pack(fill=tk.X, expand=True)

    def select_and_process_files(self):
        """Opens a file dialog to select files and then processes them."""
        self.root.withdraw() # Hide the main selection window
        try:
            files_to_process = filedialog.askopenfilenames(
                title="Select WellCAD files",
                filetypes=[("WellCAD files", "*.wcl *.WCL *.Wcl"), ("All files", "*.*")]
            )
            ...
            hs = [f for f in files_to_process if "_hs_m" in f]
            rest = [f for f in files_to_process if "_hs_m" not in f]
            tvfiles = hs + rest
            ...
            # This directory is used for the final summary, can be the parent dir of the first file
            summary_dir = os.path.dirname(tvfiles[0]) if tvfiles else '.'
            ...
            for tvfile in tvfiles:
                ...
                outfile = process_borehole_file(tvfile, app)

                if outfile and 'NEED_MANUAL_AZI_EDIT_ROTATE' in outfile:
                    rejected_azimuth_files.append(os.path.basename(outfile))

            print_summary(summary_dir, rejected_azimuth_files)
        
        finally:
            self.root.destroy() # Close the app after processing or cancellation

def main():
    """Main function to launch the GUI for file selection and processing."""
    root = tk.Tk()
    app = MainApp(root)
    root.mainloop()
