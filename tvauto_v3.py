    ...
    for tvfile in tvfiles:
        ...
        if '_hs_m' in filename:
            ...
            if not no_rotation:
                ...
                # ---------------- GraphApp with additional Tilt subplot ----------------
                class GraphApp:
                    def __init__(self, root):
                        ...
                        # Prepare Tilt data from processed tilttable (after log_editor)
                        ...
                        # Figure with two vertically stacked subplots sharing x
                        ...
                        # Add a small 'Median' label just inside the left edge
                        self.ax_tilt.text(
                            0.01,                    # 1% from left
                            self.tilt_median,
                            "Median tilt",
                            ...
                            va='bottom',
                            ha='left',
                            transform=self.ax_tilt.get_yaxis_transform(),  # x in axes fraction (0..1), y in data coords
                        ...
                        self.ax.set_xlabel("Depth (m)", fontsize = 12)  # Ensure depth axis on azimuth plot
                        self.ax.tick_params(axis='both', labelsize=11)

                        # Tilt axis scaling with small margin
                        ...
                        self.ax_tilt.set_xlabel("Depth (m)", fontsize = 12)  # Depth axis also on tilt plot
                        ...
                        ttk.Button(self.actionframe, text="ACCEPT - QC result in Wellcad", command=self.accept).pack(side=tk.LEFT, fill="x", expand=1, padx=(16,8))
                        ttk.Button(self.actionframe, text="REJECT - Manually smooth + rotate in Wellcad", command=self.reject).pack(side=tk.LEFT, fill="x", expand=1, padx=(8,16))
