...
def alpha_trim_filter(depth, azi_deg, window_m=2.0, alpha=0.50, passes=2):
    ...
    Args:
        ...
        window_m (float, optional): The size of the moving window in meters. Defaults to 2.0.
...
def create_main_gui():
    ...
    input_label = ttk.Label(main_frame, text="Input Files:")
    input_label.grid(row=0, column=0, columnspan=2, sticky=tk.W, pady=(0, 2))
    
    input_entry = ttk.Entry(main_frame, width=50, state='readonly')
    input_entry.grid(row=1, column=0, sticky=(tk.W, tk.E), padx=(0, 5))
    
    input_button = ttk.Button(main_frame, text="...", command=select_input_files, width=4)
    input_button.grid(row=1, column=1, sticky=tk.W)
    main_frame.columnconfigure(0, weight=1)

    # Output directory selection
    output_label = ttk.Label(main_frame, text="Output Folder:")
    output_label.grid(row=2, column=0, columnspan=2, sticky=tk.W, pady=(10, 2))
    
    output_entry = ttk.Entry(main_frame, textvariable=output_dir, width=50, state='readonly')
    output_entry.grid(row=3, column=0, sticky=(tk.W, tk.E), padx=(0, 5))
    
    output_button = ttk.Button(main_frame, text="...", command=select_output_dir, width=4)
    output_button.grid(row=3, column=1, sticky=tk.W)

    # Process button
    process_button = ttk.Button(main_frame, text="PROCESS", command=start_processing)
    process_button.grid(row=4, column=0, columnspan=2, pady=(15, 5), sticky=(tk.W, tk.E))
...
def print_summary(processed_files, rejected_files, missing_tilt_files, missing_caliper_files):
    """
    Prints a summary of the processing run, including any unprocessed or
    rejected files, and files with missing logs.

    Args:
        processed_files (list): Paths of successfully processed files.
        rejected_files (list): Basenames of files rejected by the user.
        missing_tilt_files (list): Basenames of files missing the TILT log.
        missing_caliper_files (list): Basenames of files missing the CAL log.
    """
    processed_count = len(processed_files)
    accepted_files = [os.path.basename(f) for f in processed_files if os.path.basename(f) not in rejected_files]

    print("\n################################################################")
    print("COMPLETE. SUMMARY : \n")

    if accepted_files:
        print("ACCEPTED (AZI ROTATION APPLIED):")
        for f in accepted_files:
            print(f"- {f}")
        print()

    if rejected_files:
        print("REJECTED (NO AZI FILTER / ROTATION. REVIEW MANUALLY IN WELLCAD):")
            ...
            class GraphApp:
                ...
                def _setup_plots(self):
                    ...
                    if self.tilt_data_available:
                        ...
                        if self.tilt_median is not None:
                        ...
                        tmin = min(self.valid_tilt_vals)
                        tmax = max(self.valid_tilt_vals)
                        if tmin == tmax:
                            tmin -= 1; tmax += 1
                        margin = 0.05 * (tmax - tmin if tmax != tmin else 1)
                        self.ax_tilt.set_ylim(tmin - margin, tmax + margin)
                    else:
                        self.ax_tilt.text(0.5, 0.5, 'TILT LOG NOT FOUND',
                ...
                def _layout_widgets(self):
                    ...
                    ttk.Radiobutton(self.algoframe, text="LOWESS (Struggles with big spikes. Good for multi-modal azi)    ",
                                    variable=self.filter_mode, value='lowess',
                                    command=self.update_plot, style="App.TRadiobutton").pack(side=tk.LEFT, padx=4)
                    ttk.Radiobutton(self.algoframe, text="Alpha-Trim (Struggles with big spikes. Good for multi-modal azi)    ",
                ...
                def _layout_dbscan_params(self):
                    """Lays out widgets specific to the DBSCAN algorithm."""
                    ...
                    self.post_dbscan_window_spin = ttk.Spinbox(self.post_dbscan_alpha_params, from_=1.0, to=40.0, increment=1, textvariable=self.alpha_window_m, width=5, format="%.1f", command=self._update_post_dbscan_alpha, font=Font(family='Segoe UI', size=11))
                    ...
                    self.dbscan_message = ttk.Label(self.paramframe, text=("Avoid using large # samples if it alters background trend.\n" "Recommend : Start small, gradually increase samples until big mag spikes removed.\n" "Then smooth residual jitter using post-proc alpha trim."), style="App.TLabel", wraplength=1000, foreground="#a73f00")
                ...
                def _layout_alpha_params(self):
                    ...
                    self.window_spin = ttk.Spinbox(self.alpha_params, from_=1.0, to=40.0, increment=1, textvariable=self.alpha_window_m, width=5, format="%.1f", command=self._update_alpha_curve, font=Font(family='Segoe UI', size=11))
    ...
    for tvfile in tvfiles:
        ...
        if outfile:
            ...
            if 'tilt' in missing_logs:
                missing_tilt_files.append(out_basename)
            if 'caliper' in missing_logs:
                missing_caliper_files.append(out_basename)


    print_summary(processed_files_paths, rejected_azimuth_files, missing_tilt_files, missing_caliper_files)
